...
class Orientation:
    ...
    def dot1(self, v: 'Vec3') -> 'Vec3':
        """Returns dot(Orientation, Vec)."""
        return Vec3(
            v.x * self.forward.x + v.y * self.right.x + v.z * self.up.x,
            v.x * self.forward.y + v.y * self.right.y + v.z * self.up.y,
            v.x * self.forward.z + v.y * self.right.z + v.z * self.up.z,
        )

    def dot2(self, v: 'Vec3') -> 'Vec3':
        """Returns dot(Vec, Orientation)."""
        return Vec3(
            v.x * self.forward.x + v.y * self.forward.y + v.z * self.forward.z,
            v.x * self.right.x   + v.y * self.right.y   + v.z * self.right.z,
            v.x * self.up.x      + v.y * self.up.y      + v.z * self.up.z,
        )
