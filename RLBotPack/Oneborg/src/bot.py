...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def fix_reversing(val, target):
            if get_angle(car_direction, car_velocity) > math.pi / 2:
                return val * clamp(math.pi - get_angle(car_direction, target), 0, abs(val)) * safe_div(abs(val))
            else:
                return val
        # Get send location
        def get_send():
        ...
        def read_players(offset, team_type):
            ...
            nearest_k = 1
            for other_p in packet.game_cars:
                t = predict_surface_bounce(Vec3(other_p.physics.location), Vec3(other_p.physics.velocity), 20, -packet.game_info.world_gravity_z)
                o_pos = Vec3(other_p.physics.location) + Vec3(other_p.physics.velocity) * t + Vec3(0, 0, packet.game_info.world_gravity_z / 2) * t**2
                if o_pos.z > 0:
            ...
            for i in range(len(packet.game_cars)):
                if i != self.index and packet.game_cars[i].physics.location.z > 0 and packet.game_cars[i].has_wheel_contact == True:
                    ...
                    if on_course == True and dist <= 254.04 + (vel - vel2).length() / 2 and (vel.length() <= vel2.length() or packet.game_cars[i].team != self.team) and ((pos2 - pos) * safe_div((pos2 - pos).length()) * (ball_location - pos).length() - (ball_location - pos)).length() > self.ball_size_setting:
                        if my_car.has_wheel_contact == True:
                            self.jumping = 2 / 60
                        elif get_angle(car_velocity, surface_pos(car_location, self.dropshot) - car_location) < math.pi / 2 and (vel2 - vel).length() <= 300 and my_car.double_jumped == False:
                            self.jumping = 0
                            override = self.generic_flip(packet, -flip_value(math.cos(get_angle(dir_convert(Vec3(side_direction.x, side_direction.y, 0)), dir_convert(Vec3(packet.game_cars[i].physics.location.x - car_location.x, packet.game_cars[i].physics.location.y - car_location.y, 0))))) * sign(roof_direction.z), -flip_value(math.cos(get_angle(dir_convert(Vec3(car_direction.x, car_direction.y, 0)), dir_convert(Vec3(packet.game_cars[i].physics.location.x - car_location.x, packet.game_cars[i].physics.location.y - car_location.y, 0))))), 0, 0, 0.5)
        # Find the boost pad
        ...
        def recovery():
            ...
            if (su - spos).length() <= (sl - spos).length() and (su - spos).length() <= (sr - spos).length() and (su - spos).length() <= (sd - spos).length():
                controls.roll = clamp(-(math.pi - abs(car_rotation.roll)) * sign(car_rotation.roll), -1, 1)
            elif (sl - spos).length() <= (su - spos).length() and (sl - spos).length() <= (sr - spos).length() and (sl - spos).length() <= (sd - spos).length():
                controls.roll = clamp(-car_rotation.roll - math.pi / 2, -1, 1)
            elif (sr - spos).length() <= (sl - spos).length() and (sr - spos).length() <= (su - spos).length() and (sr - spos).length() <= (sd - spos).length():
                controls.roll = clamp(-car_rotation.roll + math.pi / 2, -1, 1)
            elif (sd - spos).length() <= (sl - spos).length() and (sd - spos).length() <= (su - spos).length() and (sd - spos).length() <= (sr - spos).length():
                controls.roll = clamp(-car_rotation.roll, -1, 1)
            if my_car.has_wheel_contact == False:
                controls.pitch, controls.yaw = aerial_control(dir_convert(surface_pos(ref_pos + (car_velocity + Vec3(0, 0, packet.game_info.world_gravity_z) * t) * safe_div((car_velocity + Vec3(0, 0, packet.game_info.world_gravity_z) * t).length()), self.dropshot) - spos), car_rotation, 0.5, True)
        # Refueling
        def refuel():
        ...
        def power_shot():
            ...
            if ((car_location + car_velocity / 30) - (ball_location + ball_velocity / 30)).length() <= 127.02 + self.ball_size_setting and ((car_location + car_velocity / 60) - (ball_location + ball_velocity / 60)).length() > 127.02 + self.ball_size_setting and not (disparity <= 120 and get_angle(tl - car_location, Vec3(nearest_enemy.physics.location) - tl) <= math.pi / 6 and nearest_enemy is not None and nearest_enemy.has_wheel_contact == True):
                self.jumping = -1/60
            # The power hit
            elif my_car.has_wheel_contact == False and self.jumping < 0 and my_car.double_jumped == False:
        ...
        def retreat(emergency):
            ...
            if my_car.has_wheel_contact == True and (car_location - tl).length() > car_velocity.length() * 0.2 + (car_velocity.length() + 292) * 1.5:
                if (my_car.boost == 0 or not emergency) and get_angle(tl - car_location, car_velocity) <= 0.1 and car_velocity.length() > 1000 and surface_pos(car_location, self.dropshot).z == 0 and get_angle(car_velocity, car_direction) <= math.pi / 2:
                    override = self.generic_flip(packet, 0, -1, 8/60, 1/60, 0.5)
        ...
        # Catch
        def catch():
            bounce = next_bounce(packet.game_info.seconds_elapsed, ball_prediction, car_location, 2300, clamp(30 + car_velocity.length() / 70 - (ball_velocity - car_velocity).length() / 50, 0, math.inf), mid_location, self.dropshot, packet)
            tl = Vec3(predict_ball(bounce).physics.location)
            tl = tl - dir_to_point(tl, mid_location) * clamp(30 + car_velocity.length() / 70 - (ball_velocity - car_velocity).length() / 50, 0, math.inf)
            # Throttle
            if bounce == 0:
                controls.throttle = 1
            else:
            ...
            if my_car.has_wheel_contact == True:
                ...
                if ((car_location + car_velocity / 60) - (ball_location + ball_velocity / 60)).length() <= self.ball_size_setting + 20 <= (car_location - ball_location).length():
                    if self.jumping > 0:
                        self.jumping = -1 / 60
                    else:
                        override = self.generic_flip(packet, random.choice([-1, 0, 1]), random.choice([-1, 0, 1]), 8/60, 1/60, 0.5)
            controls.steer = fix_reversing(steer_toward_target(my_car, tl), tl)
            return tl
        # Nudge
        def nudge():
        ...
        def duel():
            ...
            # Other variables
            air_t = predict_surface_bounce(car_location, car_velocity, 20, -packet.game_info.world_gravity_z)
            boost_pad = find_boost_pad(tl)
            cond_hold = abs(sp.x) > (893 - self.ball_size_setting * 2 + abs(sp.y - send_location.y)) and predict_ball(5).physics.location.x * sign(sp.x) <= (893 - self.ball_size_setting * 2 + abs(sp.y - send_location.y))
            d_to_boost_goal = (boost_pad - car_location).length() + (boost_pad + send_location).length()
            # Complimentary mode
            if mode != "Aerial" and ((my_car.has_wheel_contact == False and get_angle(car_velocity - ball_velocity, ball_location - car_location) > math.pi / 2) or get_angle(car_direction, car_velocity) > math.pi / 8):
                mode_comp = "Recovery"
                ...
                mode_comp = "Refuel"
            elif mode not in ["Aerial", "Retreat", "Retreat_E"] and nearest_et < nearest_ft - 0.1 and (car_location - ball_location).length() * safe_div((car_velocity - ball_velocity).length()) >= get_angle(car_direction, ball_location - car_location) and intersection > jt + get_angle(car_velocity, ball_location - car_location):
                mode_comp = "Shadow"
        ...
        def duel_dropshot():
            mode, mode_comp = "", ""
            # Fundamental mode
            if aerial_intersection > 0 and ((Vec3(predict_ball(aerial_intersection).physics.location) - surface_pos_twox(Vec3(predict_ball(aerial_intersection).physics.location), car_location, max_jump_height(-packet.game_info.world_gravity_z, True), self.dropshot)).length() > max_jump_height(-packet.game_info.world_gravity_z, True) and not (nearest_enemy is not None and nearest_et - 0.2 > nearest_ft)):
                mode = "Aerial"
            else:
            ...
            if mode != "Aerial" and ((my_car.has_wheel_contact == False and get_angle(car_velocity, surface_pos(car_location, self.dropshot) - car_location) <= math.pi / 2) or get_angle(car_direction, car_velocity) > math.pi / 8):
                mode_comp = "Recovery"
            elif mode not in ["Aerial", "Retreat", "Retreat_E"] and nearest_et > nearest_ft + 0.2 and intersection > jt + get_angle(car_velocity, ball_location - car_location):
                mode_comp = "Setup"
            '''
        ...
        def get_modes_comp(mode_comp, target_location):
            ...
            if mode_comp == "Recovery":
                ...
                if True:
                    ...
                    ntarget_location = setup(car_location, target_location, Vec3(nearest_enemy.physics.location), False)
        ...
        intersection = intersect_time(packet.game_info.seconds_elapsed, ball_prediction, car_location, car_velocity.length(), max_jump_height(-packet.game_info.world_gravity_z, True), self.ball_size_setting, send_location, self.dropshot, True, my_car.boost, packet)
        ...
        if "target_location" not in globals():
