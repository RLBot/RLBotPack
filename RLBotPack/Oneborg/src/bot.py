        ...
        def avoid_bump():
            for i in range(len(packet.game_cars)):
                if i != self.index and packet.game_cars[i].physics.location.z > 0:
                    ...
                    on_course = False
                    if dist > 127.02:
                        if get_angle(pos2 - pos, vel) <= math.tan(math.sqrt(127.02**2 / (dist**2 - 127.02**2))):
                            on_course = True
                    else:
                        on_course = True
                    if on_course == True and my_car.has_wheel_contact == True and dist <= 127.02 + (vel - vel2).length() / 2 and (vel.length() <= vel2.length() or packet.game_cars[i].team != self.team) and get_angle(ball_location - car_location, Vec3(packet.game_cars[i].physics.location) - car_location) > math.pi / 18:
                        self.jumping = 2 / 60
        '''
        Actions
        '''
