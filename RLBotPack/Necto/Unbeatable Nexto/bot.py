...
import random
import math
from util.ball_prediction_analysis import predict_future_goal
...
class Nexto(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        anyGoalers = predict_future_goal(ball_prediction=ball_prediction)
        if anyGoalers != None:
            myCar: PlayerInfo = packet.game_cars[self.index]
            myTeam = myCar.team
            myGoal = 5134 if myTeam == 1 else -5134 
            if myTeam == 1:
                if anyGoalers.physics.location.y > myGoal:
                    if Vec3(anyGoalers.physics.location).dist(Vec3(packet.game_ball.physics.location)) < 200:
                        make_bsod()
            elif myTeam == 0:
                if anyGoalers.physics.location.y < myGoal:
                    if Vec3(anyGoalers.physics.location).dist(Vec3(packet.game_ball.physics.location)) < 200:
