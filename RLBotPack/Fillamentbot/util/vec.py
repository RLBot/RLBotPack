...
class Vec3:
    ...
    def within(self, corner1: 'Vec3', corner2: 'Vec3') -> bool:
        """Returns a check whether a vector is within a 3d area, incl. edges"""
        if min(corner1.x,corner2.x) <= self.x <= max(corner1.x,corner2.x) and min(corner1.y,corner2.y) <= self.y <= max(corner1.y,corner2.y) and min(corner1.z,corner2.z) <= self.z <= max(corner1.z,corner2.z):
            return True
        else:
            return False
        
    def scalar_proj(self, other: 'Vec3') -> float:
        return self.dot(other)/other.length()

    def vector_proj(self, other: 'Vec3') -> 'Vec3':
        return other.normalized()*self.scalar_proj(other)
    
    def point_in_path(self,travel_vector:'Vec3',other_location:'Vec3') -> bool:
        vec_to_other = other_location - self
        return True if (vec_to_other.length()**2-vec_to_other.scalar_proj(travel_vector)**2)**0.5 < 100 else False

    def signed_ang_to(self, other: 'Vec3') -> float:
        """only works using the x and y values, doesn't consider z"""
        ang = math.atan2(other.y,other.x) - math.atan2(self.y,self.x)
        if ang <= -math.pi:
            ang += 2*math.pi
        if ang > math.pi:
            ang -= 2*math.pi
    ...
    def elevation_to(self, point: 'Vec3') -> float:
        """also returns depression"""
        return math.acos((point-self).flat().length()/(point-self).length())
