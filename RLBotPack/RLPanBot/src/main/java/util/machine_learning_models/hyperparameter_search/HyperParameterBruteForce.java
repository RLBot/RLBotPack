...
public class HyperParameterBruteForce<D extends GenericData> {
    ...
    public HyperParameterBruteForce(GenericDataList<D> dataRepresentation) {
    ...
    public void nextHypothesis() {
        if(searchState == SearchState.GLOBAL_SEARCH) {
            ...
            List<Double> biasedRandomPointInRewardFunction = getBiasedRandomPointInRewardFunction();
            biasedWeightedRandomizeHyperParameter(biasedRandomPointInRewardFunction, getClosest(biasedRandomPointInRewardFunction));
    ...
    private List<Double> getBiasedRandomPointInRewardFunction() {
        double totalReward = 0;
        List<Double> correctedRewards = new ArrayList<>();

        for (Double aDouble : rewardFunctionOutput) {
            correctedRewards.add(1 / (-aDouble));
        }
        for (Double correctedReward : correctedRewards) {
            totalReward += correctedReward;
        }
        double randomValue = Math.random()*totalReward;
        double cumulativeReward = 0;
        int index = 0;
        while(cumulativeReward <= randomValue) {
            cumulativeReward += correctedRewards.get(index);
            index++;
        }

        return rewardFunctionHyperParameters.get(index-1);
    }
