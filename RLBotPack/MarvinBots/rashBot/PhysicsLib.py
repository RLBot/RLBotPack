...
def predictPath(L0, V0, AV0, dt, tps=120, g=GRAVITY):  # tuples or numpy arrays for the arguments
...
def predictPath2(ball, dt, tps=120, g=GRAVITY):  # you can use gameball from gametickpacket as an argument.
...
def intercept(gameball, gamecar, dt, tps=120, g=GRAVITY, cf=CAR_FRICTION):
...
def intercept2(bL, bV, bAV, pL, pV, dt, tps=120, g=GRAVITY, cf=CAR_FRICTION):
...
def predict_sim(L0, V0, AV0, dt, ept=1 / 120, g=GRAVITY):
...
def predict_BallSim(L0, V0, AV0, dt, ept=1 / 120, g=GRAVITY):
...
def ballStep(L0, V0, AV0, dt, g=GRAVITY):
    return state_ba3(PhyLib.ballStep(BallState(Vector3(*L0), Vector3(*V0), Vector3(*AV0)), dt, g))


def CarStep(L0, V0, dt, g=GRAVITY, cf=CAR_FRICTION):
