...
def to_shoot(s):
    """Conditions to shoot"""
    if (s.offense and (abs(s.glinex) < 1400 or Range180(s.gta - s.gpa, 1) < 0.05)) or s.kickoff:
        return 1
...
def to_jump(s):
    ...
    if s.fd2 < 150 and 80 < s.fz < s.jumpd + max(s.boostd - s.jumpd, 0) and s.jcount > 0:
...
def to_flip(s):
    ...
    if (s.gtime > 0.05 or not s.poG) and not s.jumper and ang_dif(s.sa, s.pva, 1) < .06:
        ...
        if s.sd > min(s.pvd + 600, 2300) * (.8 + .4 * (abs(s.a) < 0.5)) and s.pvd < abs(s.dspeed) - 400:
...
def to_powerslide(s):
    ...
    if .2 < ang_dif(s.a, s.av / 9, 1) < .8 and s.av * s.steer >= 0 and s.pyv * s.throttle >= 0 and abs(s.pxv) < 500:
        return 1
