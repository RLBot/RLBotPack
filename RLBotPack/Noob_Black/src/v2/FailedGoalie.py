...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def shot_variables_defense(defensive_y = -5020):
            ...
            i = 0
            ik = 1
            car_front_intersection = None
            while True:
                temp_i = i + ik
                if sign(ball_states[temp_i].physics.location.x * x_coefficient + ball_states[temp_i].physics.location.y * y_coefficient - target_coefficient) == first_c == -sign(ball_states[temp_i].physics.velocity.x * x_coefficient + ball_states[temp_i].physics.velocity.y * y_coefficient):
                    i += ik
                    ik *= 2
                else:
                    if ik > 1:
                        ...
                        car_front_intersection = i + ik
                        break
            i = 0
            ik = 1
            goal_intersection = None
            goal_direction = sign(goal_location.y)
            while True:
                temp_i = i + ik
                if ball_states[temp_i].physics.location.y * goal_direction == first_c == -sign(ball_states[temp_i].physics.velocity.y):
                    i += ik
                    ik *= 2
                else:
                    if ik > 1:
        ...
        def shoot_ball(target, li, other_cars, flick = False, double_jump = False, aerial = False, param = ""):
            ...
            if will_aerial:
                ...
                if my_car.has_wheel_contact == True and not (og_bpos.z <= 110 and bvel.z <= 100 and packet.game_info.is_kickoff_pause == False):
                ...
                if ((round((target_t[1] - jt) * 120) <= 0 and my_car.has_wheel_contact == True) or (my_car.has_wheel_contact == False and self.prev_jump == True) or ((car_location + car_velocity / 20 + Vec3(0, 0, -325) / 400).dist(Vec3(ball_states[3].physics.location)) <= 150 and my_car.has_wheel_contact == True)) and not (og_bpos.z <= 110 and bvel.z <= 100 and packet.game_info.is_kickoff_pause == False):
                    controls.jump = True
        ...
        def goalie(li, y = -5020, car_li = None):
            ...
            # In Goal
            if car_location.flat().dist(-goal_location) < 900:
                if save_deadline >= 0:
        ...
        def general_strategy_strong():
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900) and save_deadline < 0: # Side wall clears
                ...
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False and furthest_back[0] != i:
