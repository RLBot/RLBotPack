    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def shoot_ball(target, li, other_cars, flick = False, double_jump = False, aerial = False, param = ""):
            ...
            if will_aerial:
                ...
                if (og_bpos.z <= 110 and bvel.z <= 100):
                    self.prev_chip = True
                elif wait_for_fall[1] >= 0:
                    self.prev_chip = False
                if my_car.has_wheel_contact == False:
                    ...
                    if steer_toward_target(my_car, bpos) <= 0.2:
                        controls.jump = self.prev_chip == False
                    point_toward(offset)
            elif flick == True:
                ...
                if (og_bpos.z <= 110 and bvel.z <= 100):
                    self.prev_chip = True
                elif wait_for_fall[2] >= 0:
                    self.prev_chip = False
                if my_car.has_wheel_contact == True and not (og_bpos.z <= 110 and bvel.z <= 100):
        ...
        def defense(goalie = False, fast = False):
            ...
            if car_location.dist(-goal_location) <= 1500:
                ...
                controls.steer = steer_toward_target(my_car, tl)
                if my_car.has_wheel_contact == True and abs(controls.steer) <= 0.01 and car_velocity.flat().length() >= 1000 and (car_location.flat().dist(tl.flat()) >= car_velocity.flat().length() * 1.4 + 500 * 1.5 + car_velocity.flat().length()**2 / 7000 or (best_pad != None and best_pad.is_full_boost == True)):
                    override = self.begin_front_flip(packet)
            if abs(self.just_flipped - packet.game_info.frame_num) > 78 and car_velocity.z < -10:
                recover(tl)
        ...
        def prepare_for_attack(side = 0):
            ...
            controls.handbrake = car_velocity.length() < 1000 and get_angle(car_velocity, front_direction) <= math.pi / 4 and get_angle(front_direction, tpos - car_location.flat()) > math.pi / 4 and Vec3(my_car.physics.angular_velocity).length() >= min(car_velocity.length() / 500, 2) and get_angle(tpos - car_location.flat(), front_direction) < get_angle(tpos - car_location.flat(), prev_front)
        ...
        def general_strategy_2():
            ...
            for i in other_cars[0]:
                if packet.game_cars[i].is_demolished == False and closest_to_ball[0] != i:
                    cpos = Vec3(packet.game_cars[i].physics.location)
            ...
            if closest_to_ball[0] == self.index:
                shoot_ball(target_func, li, other_cars, double_jump = li[0][2] < li[0][1], aerial = True)
        ...
        def general_strategy():
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900) and save_deadline < 0: # Side wall clears
                ...
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False:
                        ...
                        biggest_dependency = abs(cpos.y - goal_location.y) + abs(cpos.x - ball_location.x) / 5 * bool(save_deadline >= 0) > abs(ball_location.y - goal_location.y)
                        if furthest_score < furthest_back[1]:
                            furthest_back = [i, furthest_score]
                        if (closest_score <= closest_to_ball[1] and bool(biggest_dependency) >= closest_to_ball[2]) or bool(biggest_dependency) > closest_to_ball[2]:
                            closest_to_ball = [i, closest_score, bool(biggest_dependency)]
            if closest_to_ball[0] == self.index:
                li = shot_variables_2(target_func)
                shoot_ball(target_func, li, other_cars, double_jump = li[0][2] < li[0][1], aerial = True) # target_t, target_f, wait_for_fall
                ...
                self.prev_chip = False
                if ball_location.y * sign(goal_location.y) > 0 and car_location.y * sign(goal_location.y) <= 50:
        ...
        if packet.game_info.is_round_active == True:
            ...
            li = shot_variables_2(target_func)
            f = ""
            if (ball_location - car_location).flat().length() <= 50 and (ball_velocity - car_velocity).length() <= 50:
                f = "Flick"
            shoot_ball(target_func, li, flick = True, aerial = False and (my_car.has_wheel_contact == False or (ball_location - car_location).flat().length() <= 20 and (ball_velocity - car_velocity).length() <= 50), param = f) # target_t, target_f, wait_for_fall
            '''
            general_strategy()
