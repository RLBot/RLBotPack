...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def shoot_ball(target, li, other_cars, flick = False, double_jump = False, aerial = False, param = ""):
            ...
            # Get opponents
            if False:
                avg_offset = Vec3(0, 0, 0)
                offset_n = 0
                for i in other_cars[1]:
                    cpos = Vec3(packet.game_cars[i].physics.location)
                    if cpos.flat().dist(ball_location.flat()) < car_location.flat().dist(ball_location.flat()):
                        a = get_angle((cpos - ball_location).flat(), Vec3(ball_location.y - car_location.y, car_location.x - ball_location.x, 0))
                        avg_offset += Vec3(ball_location.y - car_location.y, car_location.x - ball_location.x, 0).rescale(math.sqrt(max((92.75 + 42.1 * bool(mode_index == 1))**2 - (bpos.z - og_bpos.z)**2, 0))) * (bool(a > math.pi / 2) - bool(a < math.pi / 2))
                        offset_n += 1
                if offset_n > 0:
                    avg_offset /= offset_n
                    bpos = og_bpos + avg_offset + Vec3(0, 0, bpos.z - og_bpos.z)
