...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
            ...
            for i in range(round(max_t * 60) + 1):
                ...
                if target_t[2] == max_t and hit_double_check(bpos, t): # Double Jumps
                    if bpos.z - offset_vec.z * 92.75 > max_jump_height[1] + 60:
                        ...
                        target_f[2] = bp
                if target_t[3] == max_t:
                    if aerial_d_check(bpos, t): # Aerials
                        target_t[3] = t
                        target_f[3] = bp
                    else:
                        air_t += (2 * i + 1) / 3600
        ...
        def shoot_ball(target, li, flick = False, double_jump = False, aerial = False):
            target_t, target_f, wait_for_fall = li[0], li[1], li[2]
            # Drive
            will_aerial = aerial == True and my_car.boost / 100 * 3 >= target_t[3] + 0.1 * bool(my_car.has_wheel_contact) and wait_for_fall[(1 + bool(double_jump)) * bool(not flick)] >= 0
            if will_aerial:
            ...
            ticks_charging = round(target_t[(1 + bool(double_jump)) * bool(not flick)] * 120)
            ticks_driving = round(min(max(target_t[(1 + bool(double_jump)) * bool(not flick)] - tjt, 0), 5) * 120)
            should_boost = travel_d_boostless[ticks_driving] + travel_v_boostless[ticks_driving] * (ticks_charging - ticks_driving) / 120 < d_to_travel or wait_for_fall[(1 + bool(double_jump)) * bool(not flick)] < 0
            #
            '''
            d_c = clamp(car_velocity.flat().length() * target_t[(1 + bool(double_jump)) * bool(not flick)] / car_location.flat().dist(bpos.flat()), 0, 2)
            self.renderer.draw_rect_2d(50, 450, 8, 8, True, self.renderer.black())
            self.renderer.draw_rect_2d(50, 450, round(910 * d_c), 8, True, self.renderer.green())
            self.renderer.draw_rect_2d(960, 450, 8, 8, True, self.renderer.white())
            self.renderer.draw_rect_2d(1870, 450, 8, 8, True, self.renderer.black())
            '''
            # 
            ...
            controls.boost = my_car.has_wheel_contact == True and controls.throttle == 1 and car_velocity.length() < 2299 and should_boost == True and (wait_for_fall[(1 + bool(double_jump)) * bool(not flick)] < 0 or travel_d[2] + travel_v[2] * (target_t[(1 + bool(double_jump)) * bool(not flick)] - 1 / 60) < d_to_travel)
            controls.steer = steer_toward_target(my_car, bpos.flat())
            # Powerslide in some situations
            if get_angle(front_direction.flat(), (bpos - car_location).flat()) > math.pi / 4 and (bpos - car_location).flat().length() <= 1000 + car_velocity.flat().length():
            ...
            # self.renderer.draw_string_2d(900, 450, 10, 10, str(tick_t), self.renderer.green())
            ...
            '''
            if get_angle(ball_velocity.flat(), (bpos - ball_location).flat()) <= math.pi / 20 and my_car.boost == 0 and car_velocity.flat().length() >= 1000:
                average_bv = (bpos.flat() - ball_location.flat()) * safe_div(target_t[(1 + bool(double_jump)) * bool(not flick)])
                flip_vel = min(car_velocity.flat().length() + 500, 2300)
                current_d = (ball_location.flat() + average_bv / 10).dist(car_location.flat()) - car_velocity.flat().length() / 10
                one_sec_d = (ball_location.flat() + average_bv / 10 + average_bv).dist(car_location.flat()) - car_velocity.flat().length() / 10 - flip_vel
                t = current_d * safe_div(current_d - one_sec_d)
                test_jt, test_ja = new_jump_time(max(bpos.z + bvel.z**2 / 1300, 0), True)
                if t >= 1.4 + test_jt:
                    controls.steer = steer_toward_target(my_car, ball_location.flat() + average_bv * t)
                    if abs(controls.steer) <= 0.1:
            ...
            if will_aerial:
                ...
                if ((round((target_t[1] - jt) * 120) <= 0 and my_car.has_wheel_contact == True) or (my_car.has_wheel_contact == False and self.prev_jump == True) or ((car_location + car_velocity / 20 + Vec3(0, 0, -325) / 400).dist(Vec3(ball_states[3].physics.location)) <= 150 and my_car.has_wheel_contact == True)) and not (og_bpos.z <= 110 and bvel.z <= 100):
                    controls.jump = True
            ...
            if my_car.has_wheel_contact == True:
                ...
                if packet.game_info.is_kickoff_pause == True:
                    if car_velocity.length() > 800 and car_location.dist(ball_location) > 2000 and my_car.boost > 10:
        ...
        def defense(goalie = False, fast = False):
            ...
            if car_location.dist(-goal_location) <= 1500:
                ...
                controls.throttle = 1
                controls.handbrake = get_angle(car_velocity, front_direction) <= math.pi / 4 and get_angle(front_direction, tl - car_location) > math.pi / 4 and Vec3(my_car.physics.angular_velocity).length() >= min(car_velocity.length() / 500, 2) and get_angle(tl - car_location, front_direction) < get_angle(tl - car_location, prev_front)
                controls.boost = best_pad != None and best_pad.is_full_boost == True and my_car.has_wheel_contact == True != controls.handbrake
                controls.steer = steer_toward_target(my_car, tl)
                if my_car.has_wheel_contact == True and abs(controls.steer) <= 0.01 and car_velocity.flat().length() >= 1000 and car_location.flat().dist(tl.flat()) >= car_velocity.flat().length() * 1.4 + 500 * 1.5 + car_velocity.flat().length()**2 / 7000:
                    override = self.begin_front_flip(packet)
            if abs(self.just_flipped - packet.game_info.frame_num) > 78 and car_velocity.z < -10:
                recover(tl)
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900) and save_deadline < 0: # Side wall clears
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False:
                        ...
                        if closest_score <= closest_to_ball[1] and ball_location.x * sign(ball_location.x) >= cpos.x * sign(ball_location.x):
                            closest_to_ball = [i, closest_score]
                ...
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False:
                        ...
                        if furthest_score < furthest_back[1]:
                            furthest_back = [i, furthest_score]
                        if closest_score <= closest_to_ball[1] and abs(cpos.y - goal_location.y) + abs(cpos.x - bpos.x) / 5 > abs(bpos.y - goal_location.y):
                            closest_to_ball = [i, closest_score]
            else:
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False:
                        ...
                        if furthest_score < furthest_back[1]:
                            furthest_back = [i, furthest_score]
                        # Get if the ball can be retrieved before it crosses too far back
                        closest_qualifier = abs(cpos.y - goal_location.y) + abs(cpos.x - ball_location.x) / 5 * bool(save_deadline >= 0) > abs(ball_location.y - goal_location.y)
                        '''
                        a = vx^2 + vy^2 - 2300^2
                        b = 2(px * vx + py * vy)
                        c = px^2 + py^2
                        ...
                        vx = ball_velocity.x
                        vy = ball_velocity.y
                        px = ball_location.x - cpos.x
                        py = ball_location.y - cpos.y
                        cross_resultp = solve_quadratic(vx**2 + vy**2 - 2300**2, 2 * (px * vx + py * vy), px**2 + py**2, 0, -1)
                        cross_resultn = solve_quadratic(vx**2 + vy**2 - 2300**2, 2 * (px * vx + py * vy), px**2 + py**2, 0, 1)
                        closest_qualifier = False
                        if cross_resultn != None and cross_resultn >= 0:
                            future_bpos = ball_location.flat() + ball_velocity.flat() * cross_resultn
                            if (future_bpos.y - cpos.y) * sign(goal_location.y) >= -abs(future_bpos.x - cpos.x) / 5 * bool(save_deadline < 0):
                                closest_qualifier = True
                        elif cross_resultp != None and cross_resultp >= 0:
                            future_bpos = ball_location.flat() + ball_velocity.flat() * cross_resultp
                            if (future_bpos.y - cpos.y) * sign(goal_location.y) >= -abs(future_bpos.x - cpos.x) / 5 * bool(save_deadline < 0):
                                closest_qualifier = True
                        '''
                        if closest_score <= closest_to_ball[1] and closest_qualifier == True:
                            closest_to_ball = [i, closest_score]
            if closest_to_ball[0] == self.index:
                li = shot_variables_2(target_func)
                ...
                defense(False, abs(packet.game_cars[furthest_back[0]].physics.location.y + goal_location.y) > 1000)
