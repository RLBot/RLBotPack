...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def clear_target_func(pos):
            return Vec3(4096 * sign(pos.x), -goal_location.y / 2, 0)

        def shot_variables_line(target, y):
                    ...
                    if np.y * y_dir < y:
                        if target_t[2] == max_t:
                            if bpos.z - offset_vec.z * 92.75 <= max_jump_height[1] + 60:
                                ...
                                if travel_d[ticks_driving] + travel_v[ticks_driving] * (ticks_charging - ticks_driving) / 120 >= dt:
                                    target_t[2] = t
                                    target_f[2] = bp
                                    if target_t[3] != max_t:
                                        air_use = False
                        if target_t[3] == max_t:
                            ...
                            if dt <= 2975 / 6 * air_t:
                                ...
                                air_t += (2 * i + 1) / 3600
            return [target_t, target_f, target_t]
            ...
            for i in range(round(max_t * 60) + 1):
                ...
                if target_t[1] == max_t and hit_d_check(bpos, t): # Hits
                    if (bpos - (target_p - bpos).rescale(134.85)).z > max_jump_height[0]:
                        ...
                        target_f[1] = bp
                if target_t[2] == max_t and hit_double_check(bpos, t): # Double Jumps
                    if (bpos - (target_p - bpos).rescale(92.75)).z > max_jump_height[1] + 60:
                        if wait_for_fall[2] == -1:
                            wait_for_fall[2] = t
                    else:
                        target_t[2] = t
        ...
        def defense(goalie = False, y = -5050):
            ...
            if my_car.boost < 100:
                ...
                for i in range(len(self.get_field_info().boost_pads)):
                    ...
                    best_score = math.inf
                    if (abs(pad.location.x) > 3200 or abs(pad.location.y) > 4200) and pad.location.y * sign(goal_location.y) >= y and Vec3(pad.location).dist(-goal_location) < car_location.dist(-goal_location) and sign(pad.location.x) == sign(car_location.x):
            ...
            if best_pad != None:
                ...
                if goalie == True:
                    ...
                    tl = Vec3(sign(car_location.x) * 900, goal_location.rescale(y).y, 0)
            # if car_location.dist(-goal_location) <= 1500:
            if car_location.dist(goal_location.rescale(y)) <= 1500:
                if car_velocity.length() <= 100 and car_location.y * sign(goal_location.y) <= -5120 + (y + 5150) * bool(goalie == False) and get_angle(front_direction, ball_location - car_location) >= 0.2:
        ...
        def agro_strategy():
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900): # Side wall clears
                ...
                for i in other_cars[0]:
                    cpos = Vec3(packet.game_cars[i].physics.location)
                    if abs(cpos.y + goal_location.y) < furthest_back[1]:
                        furthest_back = [i, abs(cpos.y + goal_location.y)]
                    if cpos.dist(ball_location) + 5000 * bool(math.sin(packet.game_cars[i].physics.rotation.yaw) * goal_location.y < 0 or cpos.flat().dist(Vec3(sign(cpos.x, 1) * 900, -goal_location.y, 0)) <= 100) <= closest_to_ball[1] and abs(cpos.y - goal_location.y) + abs(cpos.x - ball_location.x) / 5 > abs(ball_location.y - goal_location.y):
                        closest_to_ball = [i, cpos.dist(ball_location) + 5000 * bool(math.sin(packet.game_cars[i].physics.rotation.yaw) * goal_location.y < 0 or cpos.flat().dist(Vec3(sign(cpos.x, 1) * 900, -goal_location.y, 0)) <= 100)]
            if closest_to_ball[0] == self.index:
                li = shot_variables_2(target_func)
                ...
                li = shot_variables_line(clear_target_func, -5050)
                if li[2] == [5, 5, 5, 5]:
                    defense(False)
                else:
                    shoot_ball(clear_target_func, li, double_jump = True)
                self.renderer.draw_string_3d(car_location, 1, 1, "Goalie", self.renderer.red())
            else:
                li = shot_variables_line(clear_target_func, -5050 * bool(ball_location.y * sign(goal_location.y) < 0))
                if li[2] == [5, 5, 5, 5]:
                    defense(False, -5050 * bool(ball_location.y * sign(goal_location.y) < 0))
                else:
                    shoot_ball(clear_target_func, li, double_jump = True)
                self.renderer.draw_string_3d(car_location, 1, 1, "Side", self.renderer.yellow())
