...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def shoot_ball(target, li, flick = False, double_jump = False, aerial = False):
            ...
            ticks_charging = round(target_t[mode_index] * 120)
            ticks_driving = round(min(max(target_t[mode_index] - tjt, 0), 5) * 120)
            should_boost = travel_d_boostless[ticks_driving] + travel_v_boostless[ticks_driving] * (ticks_charging - ticks_driving) / 120 < d_to_travel
            #
            ...
            controls.boost = my_car.has_wheel_contact == True and controls.throttle == 1 and car_velocity.length() < 2299 and should_boost == True
            ...
            if get_angle(front_direction.flat(), (bpos - car_location).flat()) > math.pi / 4 and (bpos - car_location).flat().length() <= 1000 + car_velocity.flat().length():
                if get_angle(bvel.flat(), (bpos - car_location).flat()) <= math.pi / 2:
                    controls.handbrake = Vec3(my_car.physics.angular_velocity).length() > min(1.5, car_velocity.length() / 700)
            ...
            if bpos.y * sign(goal_location.y) <= -5030 and abs(bpos.x) >= 1000 and abs(car_location.x) < 900:
                ...
                if (rel_pos - car_location).flat().rescale((bpos - car_location).flat().length()).y * sign(goal_location.y) > bpos.y * sign(goal_location.y):
                    ...
                    controls.handbrake = Vec3(my_car.physics.angular_velocity).length() > min(1.5, car_velocity.length() / 700) and get_angle(front_direction, rel_pos - car_location) > math.pi / 4 and get_angle(rel_pos - car_location, front_direction) < get_angle(rel_pos - car_location, prev_front)
            # Jumping
            if will_aerial:
                ...
                if self.just_flipped == False and car_velocity.z < -10:
                    recover(bpos)
                jt, ja = new_jump_time(max(bpos.z - 130, 0), False)
                ...
                if my_car.double_jumped == True and car_velocity.z > 0:
                    if self.prev_wait == True:
                        ...
                        target_roof_dir = (bpos - car_location).flat()
            ...
            if my_car.has_wheel_contact == True:
                ...
                if packet.game_info.is_kickoff_pause == True:
                    if car_velocity.length() > 800 and car_location.dist(ball_location) > 2000:
            ...
            self.renderer.draw_string_2d(40, 40, 10, 10, str(self.prev_wait), self.renderer.green())
