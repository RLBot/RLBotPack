...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def shoot_ball(target, li, flick = False, double_jump = False, aerial = False):
            ...
            if flip_ball.flat().dist(ball_location.flat() + ball_velocity.flat() * tick_t / 60) <= (ball_velocity.flat().length() * tick_t / 60) * math.tan(math.pi / 20):
                test_jt, test_ja = new_jump_time(max(flip_ball.z + flip_ballv.z**2 / 1300, 0), True)
                if t >= 1.4 + test_jt and car_velocity.flat().length() >= 1000 + 1400 * bool(my_car.boost > 0):
            ...
            if (abs(ball_location.y + goal_location.y) <= 1000 and abs(ball_location.x) >= 900): # Side wall clears
                ...
                for i in other_cars[0]:
                    if packet.game_cars[i].is_demolished == False:
                        ...
                        if furthest_score < furthest_back[1]:
                            furthest_back = [i, furthest_score]
                        if closest_score <= closest_to_ball[1] and abs(cpos.y - goal_location.y) + abs(cpos.x - ball_location.x) / 5 * bool(save_deadline >= 0) > abs(ball_location.y - goal_location.y):
                            closest_to_ball = [i, closest_score]
