...
class MyBot(BaseAgent):
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        def target_func_clear(pos):
            return Vec3(10000 * sign(pos.x - car_location.x), goal_location.y, 0)
        ...
        def shot_variables_wall(target):
            ...
            #
            def hit_d_check(bpos, t):
                np = bpos - offset_vec * 134.85 + Vec3(0, 0, 325 * t**2)
                ...
                return travel_d[ticks_driving] + travel_v[ticks_driving] * (ticks_charging - ticks_driving) / 120 >= dt
            def hit_double_check(bpos, t):
                np = bpos - offset_vec * 92.75 + Vec3(0, 0, 325 * t**2)
                dt = used_f(car_location, np).dist(used_f2(car_location))
        ...
        def shot_variables_line(target, p1, p2):
            ...
            td = [travel_d_boostless, travel_d]
            tv = [travel_v_boostless, travel_v]
            target_p = target(ball_location)
            offset_vec = (target_p - ball_location).normalized()
            single_d = cross_line_check(ball_location, 134.85)
            double_d = cross_line_check(ball_location, 92.75)
            # Get the best prediction to shoot at
            target_t = [[max_t, max_t], [max_t, max_t], 10, 10]
            time_boost = [True, True]
            ...
            for i in range(round(max_t * 60) + 1):
                ...
                target_p = target(bpos)
                offset_vec = (target_p - bpos).normalized()
                if target_t[0][0] == max_t:
                    bl = bool(time_boost[0])
                    if hit_d_check(bpos, td[bl], tv[bl], t):
                        if bpos.z - offset_vec.z * 134.85 > max_jump_height[0]:
                            if wait_for_fall[0] == -1:
                                wait_for_fall[0] = t
                        else:
                            ...
                            target_f[0][bl] = bp
                            time_boost[0] = False
                if target_t[1][0] == max_t:
                    bl = bool(time_boost[1])
                    if hit_double_check(bpos, td[bl], tv[bl], t):
                        if bpos.z - offset_vec.z * 92.75 > max_jump_height[1] + 60:
                            if wait_for_fall[1] == -1:
                                wait_for_fall[1] = t
                        else:
        ...
        def clear(target, li, double_jump):
            # Get variables
            # Time / Frame: [[v, v], [v, v], v, v]
            # Wait: [v, v]
            target_t, target_f, wait_for_fall = li[0], li[1], li[2]
            used_t_shot = target_t[bool(double_jump)]
            used_t_line = target_t[2 + bool(double_jump)]
            used_f_shot = target_f[bool(double_jump)]
            used_f_line = target_f[2 + bool(double_jump)]
            used_wait_shot = wait_for_fall[bool(double_jump)]
            # Whether it can intersect the line
            og_bpos = Vec3(used_f_line.physics.location)
            bvel = Vec3(used_f_line.physics.velocity)
            if double_jump == True:
                bpos = og_bpos - (target(og_bpos) - og_bpos).rescale(92.75)
            else:
                bpos = og_bpos - (target(og_bpos) - og_bpos).rescale(134.85)
            # Forward
            d_to_travel = bpos.flat().dist(car_location.flat())
            tjt, tja = new_jump_time(max(bpos.z, 0), double_jump)
            ticks_charging = round(used_t_line * 120)
            ticks_driving = round(min(max(used_t_line - tjt, 0), 5) * 120)
            can_intersect_forward = travel_d[ticks_driving] + travel_v[ticks_driving] * (ticks_charging - ticks_driving) / 120 < d_to_travel
            can_intersect_backward = travel_d_boostless[ticks_driving] + travel_v_boostless[ticks_driving] * (ticks_charging - ticks_driving) / 120 < d_to_travel
            # If it can hit the ball on the line
            can_meet_on_line = False
            if get_angle(front_direction, bpos - car_location) < math.pi / 2 and can_intersect_forward == True:
                can_meet_on_line = True
            elif can_intersect_backward == True:
                can_meet_on_line = True
            # 
            danger = used_t_line <= 6
            if danger == True:
                if my_car.has_wheel_contact == True:
                    self.prev_wait = bpos.z > max_jump_height[1]
                    controls.steer = steer_toward_target(my_car, bpos)
                    controls.throttle = sign(d_to_travel - car_velocity.length() * sign(math.pi / 2 - get_angle(car_velocity, bpos - car_location)) * used_t_line) * sign(math.pi / 2 - get_angle(front_direction, bpos - car_location))
                    controls.boost = controls.throttle == 1 and sign(math.pi / 2 - get_angle(front_direction, bpos - car_location)) == 1
                # Jumping
                if double_jump == True:
                    jt, ja = new_jump_time(max(bpos.z, 0), True)
                    if my_car.has_wheel_contact == True:
                        controls.jump = round((used_t_line - jt) * 120) <= 0
                    else:
                        if abs(packet.game_info.frame_num - self.jump_tick) < 24:
                            controls.jump = True
                        else:
                    ...
                    jt, ja = new_jump_time(max(bpos.z, 0), False)
                    if ((round((used_t_line - jt) * 120) <= 0 and my_car.has_wheel_contact == True) or (my_car.has_wheel_contact == False and self.prev_jump == True) or ((car_location + car_velocity / 20 + Vec3(0, 0, -325) / 400).dist(Vec3(ball_states[3].physics.location)) <= 150 and my_car.has_wheel_contact == True)) and self.should_chip == False:
                        controls.jump = True
                    if my_car.has_wheel_contact == False:
                        if (car_location + car_velocity / 60 + Vec3(0, 0, -325) / 3600).dist(Vec3(ball_states[1].physics.location)) <= 150 or packet.game_ball.latest_touch.player_index == self.index != self.prev_touch[0]:
                            controls.jump = not self.prev_jump
                ...
                if abs(car_location.y + goal_location.y) <= 92.75:
                    controls.throttle = sign(get_angle(car_velocity.flat(), front_direction.flat()) - math.pi / 2)
                    controls.steer = steer_toward_target(my_car, car_location + Vec3(sign(front_direction.x, -car_location.x), 0, 0))
                    if abs(front_direction.y) > 0.1 + math.sqrt(abs(my_car.physics.angular_velocity.z)) / 3 * bool(sign(my_car.physics.angular_velocity.z) == sign((my_car.physics.rotation.yaw % math.pi) - math.pi / 2)) and car_velocity.flat().length() <= 40:
                        controls.handbrake = True
                    if controls.handbrake == True:
                        controls.steer = controls.steer * controls.throttle
                else:
                    controls.steer = steer_toward_target(my_car, Vec3(sign(car_location.x) * min(car_location.flat().dist(-goal_location) / 2, 1500), -goal_location.y, 0))
                    controls.throttle = 1
                    if my_car.has_wheel_contact == True and abs(controls.steer) <= 0.01 and car_velocity.flat().length() >= 1000 and abs(car_location.y + goal_location.y) + 50 >= (car_velocity.flat().length() * 1.4 + 500 * 1.5 + car_velocity.flat().length()**2 / 7000) * abs(front_direction.y):
                        override = self.begin_front_flip(packet)
        ...
        def general_strategy():
            ...
            if closest_to_ball[0] == self.index:
                roll_up_wall_condition = ball_location.z + ball_velocity.z**2 / 1300 <= 130 and str(nbw) != "nan" and car_location.flat().dist(air_nbw) > travel_d[min(round(nbw) * 120, 599)] and ((4096 - abs(ball_location.x) < 5120 - abs(ball_location.y) and abs(ball_velocity.x)**2 / 1300 >= 400) and (4096 - abs(ball_location.x) >= 5120 - abs(ball_location.y) and abs(ball_velocity.y)**2 / 1300 >= 400 and abs(car_rotation.roll) > math.pi / 4 and ball_location.y * goal_location.y < 0)) and (ball_velocity.z > 0 or ball_location.z >= 400)
                stable_up_wall_condition = new_jump_time_wall(min(4096 - abs(ball_location.x), 5120 - abs(ball_location.y)), 0)[0] <= 1.45 and ((4096 - abs(ball_location.x) < 5120 - abs(ball_location.y) and abs(ball_velocity.x) < 150) or (4096 - abs(ball_location.x) >= 5120 - abs(ball_location.y) and abs(ball_velocity.y) < 150 and abs(car_rotation.roll) > math.pi / 4 and ball_location.y * goal_location.y < 0)) and ball_location.z + ball_velocity.z**2 / 1300 <= 1900 and (ball_velocity.z > 0 and ball_location.z + ball_velocity.z**2 / 1300 >= 400 or ball_location.z >= 400)
                ...
                if go_up_wall == True:
                    ...
                    self.renderer.draw_string_3d(car_location, 1, 1, "Shoot (Wall)", self.renderer.green())
                else:
                    if packet.game_info.is_kickoff_pause == True:
                        tf = target_func_flat
                    else:
        ...
        if packet.game_info.is_round_active == True:
            general_strategy()
            # li = shot_variables_line(target_func, -goal_location + Vec3(-900, 0, 0), -goal_location + Vec3(900, 0, 0))
            # clear(target_func, li, True)
