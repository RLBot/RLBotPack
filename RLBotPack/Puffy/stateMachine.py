...
class StateMachine: 
	...
	def restartStateAndContinueTick(self, packet, *args) -> bool:
		self.currentState = self.currentState.__class__(self.agent, *args)
	...
	def selectState(self, packet: GameTickPacket):
		if self.agent.spikeWatcher.carrying_car == packet.game_cars[self.agent.index]:
			self.currentState = Spiked(self.agent)
		else:
			self.currentState = Chase(self.agent)
