...
from rlutilities.linear_algebra import vec3, axis_to_rotation, dot, norm, normalize, flatten
...
class Dribble(Recover):
	...
	def tick(self, packet: GameTickPacket) -> bool:
		...
		realBallVelocity = ballVelocity = Vec3(packet.game_ball.physics.velocity)
		...
		for carIndex in range(packet.num_cars):
			...
			if car.team == self.agent.team and carIndex != self.agent.index and not car.is_demolished:
				...
				if otherPositionScore + math.copysign(5, carIndex - self.agent.index) < positionScore:
					...
					teamHasBallControl = teamClosestDistance - 500 < enemyClosestDistance and ballLocation.y * (2*self.agent.team-1) < 0
		...
		if not goingForBoost:
			...
			if carToTargetLocation.x > 300 and not beAnnoying:
				if carLocation.y * (2*self.agent.team-1) > -1000 and ballToCarAbsoluteLocation.y * (2*self.agent.team-1) > 0:
					time = (5120 - ballLocation.y * (2*self.agent.team-1)) / max(1, -ballToCarAbsoluteLocation.y * (2*self.agent.team-1))
			...
			if ballLocation.z > 160 or ballToCarLocation.length() > 800:
				...
				self.steerBiasLimit = min(1.5, 1 + 1 * max(0, carSpeed - 600) / 1800, self.steerBiasLimit + 0.065)
		...
		if myCar.has_wheel_contact and throttle > 1.7 and carLocation.z < 100:
			...
			if realBallLocation.z > 500:
				...
				topHeight = max(0, realBallLocation.z - 92 + 1/2 * -packet.game_info.world_gravity_z * afterTopTime**2)
				timeForBallToReachGround = max(1/120, math.sqrt(2 * topHeight) - afterTopTime)
				nextBouncePosition = realBallLocation + timeForBallToReachGround * realBallVelocity
				nextBouncePosition = vec3(nextBouncePosition.x, nextBouncePosition.y, nextBouncePosition.z)
				minimumSpeedRequired = min(minimumSpeedRequired, norm(flatten(self.agent.car.position - nextBouncePosition)) / timeForBallToReachGround)

			self.controller.boost = (carSpeed < minimumSpeedRequired) and myCar.boost > 0
		else:
