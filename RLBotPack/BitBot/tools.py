                ...
                for pair in targets:
                    ...
                    if fits:
                        ...
                        if True:  # in_field(ball_location - (200*best_shot_vector),1):
                            ...
                            if forward_flag:
                                if (ball_location[2] <= 300 or (
                                        not in_field(ball_location, 200) and not in_field(agent.me.location,
                                                                                          100))) and slope > 0.0:
                                    hits[pair].append(jump_shot(ball_location, intercept_time, best_shot_vector, slope))
                                if ball_location[2] > 325 and slope > 1 and cap(ball_location[2] - 400, 100,
                                                                                2000) * 0.1 < agent.me.boost:
                                    if abs((
                                                   car_to_ball / forward_time) - agent.me.velocity).magnitude() - 300 < 400 * forward_time:
                                        hits[pair].append(
